# Claude Code Project Instructions — bitflags-kotlin

## Project Overview

This is **bitflags-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`bitflags`](https://crates.io/crates/bitflags).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/bitflags/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/bitflags>
...
Targets: macOS arm64, Linux x64, mingw-x64, iOS arm64/x64/simulator-arm64,
tvOS arm64/simulator-arm64, watchOS arm32/arm64/device-arm64/simulator-arm64,
Android, Android Native, JVM, JS, Wasm-JS, and Wasm-WASI.
